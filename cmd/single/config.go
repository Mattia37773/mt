...
func configSingle(args []string) {
	generateConfigFile()
	fmt.Println(text.Green("Configfile generated"))
}

func generateConfigFile() {
	_, err := os.Stat(".mt.yaml")
	if !os.IsNotExist(err) {
		fmt.Print(text.Red("Error: Config file already exists\n"))
		os.Exit(1)
	}

	file, err := os.Create(".mt.yaml")
	if err != nil {
		fmt.Print(text.Red("Error: Something went wrong with creating the config File\n"))
		os.Exit(1)
	}
