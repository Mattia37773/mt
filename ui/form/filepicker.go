...
func FilePicker(startDir string) (string, error) {
	...
	for {
		...
		if currentDir != "/" && currentDir != startDir {
			options = append(options, huh.NewOption(text.Lime(".."), ".."))
		}
		...
		for _, entry := range entries {
			...
			if entry.IsDir() && !isArchive(name) {
				options = append(options, huh.NewOption(text.GlowPink(name), fullPath))
			} else {
				options = append(options, huh.NewOption(text.Purple(name), fullPath))
			}
		}
